...
contract LaunchFeeForkTest is Test {
  ...
  function setUp() public {
    ...
    plugin = new HydrexBasePlugin(address(pool), address(FACTORY), address(this), AlgebraFeeConfiguration(0, 0, 0, 0, 1, 1, 10000), 10000);
  ...
  function _inputSwap(bool direction, bool prepay) internal returns (uint256 output) {
    ...
    if (prepay) {
      (amount0, amount1) = pool.swapWithPaymentInAdvance(address(this), address(this), direction, 100 ether, limit, '');
    } else {
  ...
  function test_RealPoolChargesDecayToken0ToToken1() public { _assertSchedule(true, false); }
  function test_RealPoolChargesDecayToken1ToToken0() public { _assertSchedule(false, false); }
  function test_PrepaymentCannotBypassLaunchFee() public { _assertSchedule(true, true); }
