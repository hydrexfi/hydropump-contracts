...
contract PluginPoolMock {
    ...
    function setPlugin(address value) external {
        plugin = value;
    }

    function setPluginConfig(uint8 value) external {
        config = value;
    }
    ...
    function setFee(uint16 value) external {
        fee = value;
    }
...
contract PluginAuthorityMock {
    ...
    function hasRoleOrOwner(bytes32, address user) external view returns (bool) {
        return user == owner;
    }

    function setFarmingAddress(address user) external {
        farmingAddress = user;
    }
}
...
contract SecurityRegistryMock {
    ...
    function setStatus(ISecurityRegistry.Status value) external {
        status = value;
    }

    function getPoolStatus(address) external view returns (ISecurityRegistry.Status) {
        return status;
    }
}
...
contract VirtualPoolMock {
    ...
    function crossTo(int24 tick, bool direction) external returns (bool) {
        lastTick = tick;
...
contract HydrexBasePluginTest is Test {
    ...
    function _swap(bool direction) internal returns (uint24 fee) {
        ...
        (bytes4 selector, uint24 overrideFee, uint24 pluginFee) =
            plugin.beforeSwap(address(this), address(this), direction, 1 ether, 0, false, "");
    ...
    function test_NoOverridesWhenFeeModesDisabled() public {
        assertEq(_swap(true), 0);
    }
    ...
    function test_FeeAndSecuritySettingsRequireAuthorization() public {
        ...
        plugin.changeSlidingFeeStatus(true);
        vm.expectRevert();
        plugin.changeFeeConfiguration(_config(5000));
        vm.expectRevert();
        plugin.setSecurityRegistry(address(1));
        vm.stopPrank();
