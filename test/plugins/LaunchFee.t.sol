...
contract LaunchFeeTest is HydrexBasePluginTest {
    ...
    function test_StartIsPoolInitializationNotPluginDeployment() public {
        ...
        HydrexBasePlugin other =
            new HydrexBasePlugin(address(otherPool), address(authority), address(authority), _config(3000), 4000);
    ...
    function test_ExistingPoolOracleInitializationDoesNotApplyLaunchFee() public {
        HydrexBasePlugin other =
            new HydrexBasePlugin(address(pool), address(authority), address(authority), _config(3000), 4000);
    ...
    function test_NoCreatorRouterOrExactOutputExemptions() public {
        ...
        for (uint256 i; i < 4; i++) {
            vm.prank(address(pool));
            (, uint24 fee, uint24 extra) = plugin.beforeSwap(
                address(uint160(i + 1)), address(this), i % 2 == 0, -1 ether, 0, i > 1, "arbitrary data"
            );
            assertEq(fee, 990000);
            assertEq(extra, 0);
        }
