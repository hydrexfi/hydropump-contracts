...
library AdaptiveFee {
    ...
    function expXg4(uint256 x, uint16 g, uint256 gHighestDegree) internal pure returns (uint256 res) {
        ...
        assembly {
            let xdg := div(x, g)
            switch xdg
            case 0 { closestValue := 100000000000000000000 }
            // 1
            case 1 { closestValue := 271828182845904523536 }
            // ~= e
            case 2 { closestValue := 738905609893065022723 }
            // ~= e^2
            case 3 { closestValue := 2008553692318766774092 }
            // ~= e^3
            case 4 { closestValue := 5459815003314423907811 }
            // ~= e^4
