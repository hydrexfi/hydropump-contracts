...
library VolatilityOracle {
    ...
    function write(Timepoint[UINT16_MODULO] storage self, uint16 lastIndex, uint32 blockTimestamp, int24 tick)
        internal
        returns (uint16 indexUpdated, uint16 oldestIndex)
    {
    ...
    function getSingleTimepoint(
        ...
        unchecked {
            ...
            (Timepoint storage beforeOrAt, Timepoint storage atOrAfter, bool samePoint,) =
            ...
            (uint32 timepointTimeDelta, uint32 targetDelta) =
                (timestampAfter - targetTimepoint.blockTimestamp, target - targetTimepoint.blockTimestamp);

            targetTimepoint.tickCumulative += ((tickCumulativeAfter - targetTimepoint.tickCumulative)
                    / int56(uint56(timepointTimeDelta))) * int56(uint56(targetDelta));
            targetTimepoint.volatilityCumulative += ((atOrAfter.volatilityCumulative
    ...
    function getOldestIndex(Timepoint[UINT16_MODULO] storage self, uint16 lastIndex)
        internal
        view
    ...
    function getAverageVolatility(
        ...
        unchecked {
            ...
            if (!timeAtLastTimepoint) {
                lastCumulativeVolatility =
                    _getVolatilityCumulativeAt(self, currentTime, 0, tick, lastIndex, oldestIndex);
            }
            ...
            if (_lteConsideringOverflow(oldestTimestamp, currentTime - WINDOW, currentTime)) {
                ...
                if (timeAtLastTimepoint) {
                    ...
                    (self[windowStartIndex].blockTimestamp, self[windowStartIndex].volatilityCumulative);

                    uint32 timeDeltaBetweenPoints = self[windowStartIndex + 1].blockTimestamp - oldestTimestamp;

                    cumulativeVolatilityAtStart += ((self[windowStartIndex + 1].volatilityCumulative
                                - cumulativeVolatilityAtStart)
                            * (currentTime - WINDOW - oldestTimestamp)) / timeDeltaBetweenPoints;
                } else {
                    cumulativeVolatilityAtStart =
                        _getVolatilityCumulativeAt(self, currentTime, WINDOW, tick, lastIndex, oldestIndex);
                }
    ...
    function _volatilityOnRange(int256 dt, int256 tick0, int256 tick1, int256 avgTick0, int256 avgTick1)
        internal
        pure
    ...
    function _getAverageTickCasted(
        ...
        (int256 _avgTick, uint256 _windowStartIndex) = _getAverageTick(
            self, time, tick, lastIndex, oldestIndex, lastTimestamp, lastTickCumulative
        );
        unchecked {
    ...
    function _getAverageTick(
        ...
        (uint32 oldestTimestamp, int56 oldestTickCumulative) =
            (self[oldestIndex].blockTimestamp, self[oldestIndex].tickCumulative);
        ...
        unchecked {
            ...
            if (_lteConsideringOverflow(lastTimestamp, currentTime - WINDOW, currentTime)) {
                ...
                (tickCumulativeAtStart, windowStartIndex) =
                    _getTickCumulativeAt(self, currentTime, WINDOW, tick, lastIndex, oldestIndex);
    ...
    function _getVolatilityCumulativeAt(
        ...
        unchecked {
            ...
            (uint32 timestampBefore, uint88 volatilityCumulativeBefore) =
                (beforeOrAt.blockTimestamp, beforeOrAt.volatilityCumulative);
            ...
            if (samePoint) {
                ...
                (int24 avgTick,) = _getAverageTickCasted(
                    self, target, tick, lastIndex, oldestIndex, timestampBefore, beforeOrAt.tickCumulative
                );

                return (volatilityCumulativeBefore
                        + uint88(
                            _volatilityOnRange(
                                int256(uint256(target - timestampBefore)), tick, tick, beforeOrAt.averageTick, avgTick
                            )
                        ));
            }

            (uint32 timestampAfter, uint88 volatilityCumulativeAfter) =
                (atOrAfter.blockTimestamp, atOrAfter.volatilityCumulative);
            if (target == timestampAfter) return volatilityCumulativeAfter; // we're at the right boundary

            // we're in the middle
            (uint32 timepointTimeDelta, uint32 targetDelta) =
                (timestampAfter - timestampBefore, target - timestampBefore);
            return volatilityCumulativeBefore
    ...
    function _getTickCumulativeAt(
        ...
        unchecked {
            ...
            (uint32 timestampBefore, int56 tickCumulativeBefore) =
                (beforeOrAt.blockTimestamp, beforeOrAt.tickCumulative);
            ...
            if (samePoint) {
                return
                    ((tickCumulativeBefore + int56(tick) * int56(uint56(target - timestampBefore))), _indexBeforeOrAt);
            } // if target is newer than last timepoint
        ...
        returns (Timepoint storage beforeOrAt, Timepoint storage atOrAfter, bool samePoint, uint256 indexBeforeOrAt)
    {
        ...
        (beforeOrAt, atOrAfter, indexBeforeOrAt) =
            _binarySearch(self, currentTime, target, lastIndex, oldestIndex, useHeuristic);
    ...
    function _binarySearch(
        ...
        unchecked {
            ...
            (beforeOrAt, atOrAfter, indexBeforeOrAt) =
                _binarySearchInternal(self, currentTime, target, left, right, withHeuristic);
        }
    }
                ...
                if (initializedBefore) {
                    if (_lteConsideringOverflow(timestampBefore, target, currentTime)) {
                        ...
                        (bool initializedAfter, uint32 timestampAfter) =
                            (atOrAfter.initialized, atOrAfter.blockTimestamp);
                        if (initializedAfter) {
