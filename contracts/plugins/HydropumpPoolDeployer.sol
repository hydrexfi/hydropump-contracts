...
contract HydropumpPoolDeployer is HydrexBasePluginFactory {
  ...
  constructor(address factory_, address launcher_, address settingsSource)
    HydrexBasePluginFactory(factory_)
  {
    if (factory_.code.length == 0 || launcher_.code.length == 0 || settingsSource.code.length == 0) revert InvalidConfiguration();
    launcher = launcher_;
    ...
    (uint16 a1, uint16 a2, uint32 b1, uint32 b2, uint16 g1, uint16 g2, uint16 base) = source.defaultFeeConfiguration();
  ...
  function beforeCreatePoolHook(address pool, address creator, address deployer, address token0, address token1, bytes calldata)
    external override returns (address)
  {
    if (msg.sender != algebraFactory || pendingPool == address(0) || pool != pendingPool ||
        creator != launcher || deployer != address(this) ||
        pool != IAlgebraFactory(algebraFactory).computeCustomPoolAddress(address(this), token0, token1)) revert InvalidCallback();
    return _createPlugin(pool);
  }

  function afterCreatePoolHook(address plugin, address pool, address deployer) external view override {
    if (msg.sender != algebraFactory || pool != pendingPool || deployer != address(this) ||
        plugin == address(0) || pluginByPool[pool] != plugin) revert InvalidCallback();
  }
