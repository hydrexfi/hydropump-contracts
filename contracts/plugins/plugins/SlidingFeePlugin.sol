...
abstract contract SlidingFeePlugin is AlgebraBasePlugin, ISlidingFeePlugin {
    ...
    function _getFeeAndUpdateFactors(bool zeroToOne, int24 currenTick, int24 lastTick, bool overrideFee, uint16 fee)
        internal
        returns (uint16)
    {
    ...
    function _calculateFeeFactors(int24 currentTick, int24 lastTick, uint16 priceChangeFactor)
        internal
        view
        returns (FeeFactors memory feeFactors)
    {
        ...
        int256 priceChangeRatio =
            int256(FullMath.mulDiv(sqrtPriceDelta, sqrtPriceDelta, 2 ** 96)) - int256(1 << FEE_FACTOR_SHIFT); // (currentPrice - lastPrice) / lastPrice
