...
abstract contract FarmingProxyPlugin is AlgebraBasePlugin, IFarmingPlugin {
  ...
  uint8 private constant FARMINGPROXYPLUGIN_CONFIG = uint8(Plugins.AFTER_SWAP_FLAG);
  ...
  function setIncentive(address newIncentive) external override {
    ...
    if (isPluginConnected) {
      _enablePluginFlags(FARMINGPROXYPLUGIN_CONFIG);
    }
  }
  ...
  function _updateVirtualPoolTick(bool zeroToOne) internal {
    ...
    if (_incentive != address(0)) {
      ...
      _disablePluginFlags(FARMINGPROXYPLUGIN_CONFIG); // should not be called, reset config
    }
  }
