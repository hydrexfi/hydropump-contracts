...
abstract contract DynamicFeePlugin is AlgebraBasePlugin, IDynamicFeeManager {
    ...
    uint8 private constant DYNAMICFEEPLUGIN_CONFIG = uint8(Plugins.BEFORE_SWAP_FLAG | Plugins.DYNAMIC_FEE);
    ...
    function changeDynamicFeeStatus(bool _isEnable) external override {
        require(
            msg.sender == pluginFactory
                || IAlgebraFactory(factory).hasRoleOrOwner(ALGEBRA_BASE_PLUGIN_MANAGER, msg.sender)
        );
