...
/// @notice Stops a fee swap 5% worse than the stricter of the block's opening price and the pool's
///         two-minute average, and never more than twice that from spot.
/// @dev The plugin records the tick before each block's first swap; no record this block means spot is it.
///      Skips when there is no usable plugin, or the open and the average are more than 5% apart. A pool
///      younger than the window uses the open alone.
library SwapPriceLimit {
    ...
    /// @return room False when the swap must not be sent.
